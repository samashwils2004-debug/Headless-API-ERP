...
// Google Stitch types and functions
...
export type StitchScreenMap = Record<string, StitchScreen>;

export type StitchGeneratePromptRequest = {
  system_name: string;
  modules: Array<{
    id: string;
    label: string;
...
export type StitchGenerateResponse = {
...
export async function generateStitchMockup(
  ...
  payload: StitchGeneratePromptRequest
): Promise<StitchGenerateResponse> {
  // This calls our internal Next.js API /api/stitch/generate - not the FastAPI backend.
  // No X-institution-Id / X-Project-Id / X-API-Key headers are added here —
  // the /api/stitch/generate endpoint handles auth internally.
  // We still pass the tenant/institution context because Stitch needs it
  // to know which institution's API keys to use.
  const response = await fetch("/api/stitch/generate", {
    method: "POST",
    headers: headersForTenant(_tenant),
    body: JSON.stringify(payload),
  });
  ...
  try {
    ...
    console.error("Failed to parse Stitch generate response:", e);
    throw new Error("Failed to parse Stitch generate response");
  }

  // --- quick type-assert-and-return for now, but this needs to be proper ---
  const typed = parsed as StitchGenerateResponse;
  return typed; 
}
